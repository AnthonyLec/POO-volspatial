...
class BasePanel:
    ...
    def load_data(self):
        data = self.db.load("bases")
        self.current_data = data
        entities = [Base.from_dict(item) for item in data]
    ...
    def setup_buttons(self):
        ...
        add_button = Button(self.button_frame, text="Add Base", command=self.add_item)
        add_button.pack(side="left", padx=5, pady=5)

        delete_button = Button(self.button_frame, text="Delete Base", command=self.delete_item)
    ...
    def delete_item(self):
        ...
        if not selected_item:
            messagebox.showwarning("Warning", "Please select a base to delete")
            return
        ...
        self.db.save("bases", self.current_data)
        self.load_data()
        messagebox.showinfo("Information", "Base successfully deleted")
