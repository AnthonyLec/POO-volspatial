...
class VehiclePanel:
    ...
    def load_data(self):
        data = self.db.load("vehicles")
        self.current_data = data
        entities = [Vehicle.from_dict(item) for item in data]
    ...
    def setup_buttons(self):
        ...
        add_button = Button(self.button_frame, text="Add Vehicle", command=self.add_item)
        add_button.pack(side="left", padx=5, pady=5)

        delete_button = Button(self.button_frame, text="Delete Vehicle", command=self.delete_item)
    ...
    def add_item(self):
        def save_new_item(fields):
            ...
            new_item = Vehicle.from_dict(new_data)
            self.current_data.append(new_item.to_dict())
            self.db.save("vehicles", self.current_data)
            self.load_data()
            new_item_window.destroy()

        new_item_window = Toplevel(self.root)
        center_modal(new_item_window)
        new_item_window.title("Add Vehicle")

        fields = {}
        labels = ["model", "capacity", "year_of_creation"]
        row = 0
    ...
    def delete_item(self):
        ...
        self.db.save("vehicles", self.current_data)
        self.load_data()
        messagebox.showinfo("Information", "Vehicle successfully deleted")
